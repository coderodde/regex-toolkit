...
final class RandomBinaryRegexBuilder {
    ...
    String buildRegexString(RegexTreeNode root) {
        ...
        if (root.isLeaf()) {
            switch (root.getRegexToken().type()) {
                case LITERAL -> {
                    return Character.toString(root.getRegexToken().codePoint());
                }
    ...
    private static String 
        ...
        switch (root.getRegexToken().type()) {
            ...
            case LITERAL:
                RegexTreeNode rtn = (RegexTreeNode) root;
                int cp = rtn.getRegexToken().codePoint();
    ...
    private static void constructRegexFromTreeImplSingleOperand(
        ...
        root.setRegexToken(new RegexTokenSimple(regexTokenType));
    ...
    private static RegexTokenType getDoubleOperandRegexTreeType(Random random) {
        ...
        if (coin < 0.65) {
            return RegexTokenType.CONCATENATION;
        } else {
    ...
    private static RegexToken getRandomBitRegexToken(Random random) {
        return new RegexTokenLiteral((int)(random.nextBoolean() ? '1' : '0'));
    }
    ...
    private static RegexTreeNode getRegexTreeNodeLeaf(Random random) {
        ...
        if (random.nextDouble() < 0.35) {
            regexToken = new RegexTokenSimple(RegexTokenType.DOT);
        } else {
    ...
    private static RegexToken getDoubleParameterRegexToken(Random random) {
        ...
        if (coin < 0.4) {
            return new RegexTokenSimple(RegexTokenType.CONCATENATION);
        } else {
            return new RegexTokenSimple(RegexTokenType.UNION);
        }
    }
